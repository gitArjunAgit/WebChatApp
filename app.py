...
if MONGO_URI:
    client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000, tls=True, tlsAllowInvalidCertificates=True, retryWrites=True)
...
class PongGame:
    ...
    def update(self):
        ...
        if (self.ball_x - self.ball_radius < self.paddle_width and self.p1_y < self.ball_y < self.p1_y + self.paddle_height):
            self.ball_vx = -self.ball_vx
            self.ball_x = self.paddle_width + self.ball_radius
            self.ball_vy += (((self.ball_y - self.p1_y) / self.paddle_height) - 0.5) * 5
        if (self.ball_x + self.ball_radius > self.width - self.paddle_width and self.p2_y < self.ball_y < self.p2_y + self.paddle_height):
            self.ball_vx = -self.ball_vx
            self.ball_x = self.width - self.paddle_width - self.ball_radius
            self.ball_vy += (((self.ball_y - self.p2_y) / self.paddle_height) - 0.5) * 5
        if self.ball_x < 0:
            self.p2_score += 1
            self.reset_ball()
    ...
    def get_state(self):
        return {
            ...
            'p1': {'name': self.player1_name, 'color': self.player1_color, 'y': self.p1_y, 'score': self.p1_score, 'width': self.paddle_width, 'height': self.paddle_height},
            'p2': {'name': self.player2_name, 'color': self.player2_color, 'y': self.p2_y, 'score': self.p2_score, 'width': self.paddle_width, 'height': self.paddle_height},
            'width': self.width,
            'height': self.height
        }
...
def handle_join_pong(data):
    ...
    if len(pong_queue) >= 2:
        ...
        socketio.emit('game_started', {'game_id': game.game_id, 'player_number': 1, 'opponent': p2['user'], 'opponent_color': p2['color'], 'your_color': p1['color']}, room=p1['sid'])
        socketio.emit('game_started', {'game_id': game.game_id, 'player_number': 2, 'opponent': p1['user'], 'opponent_color': p1['color'], 'your_color': p2['color']}, room=p2['sid'])
        socketio.emit('pong_queue_update', {'count': len(pong_queue)})
...
def handle_join_private(data):
    ...
    if len(private_queue) >= 2:
        ...
        socketio.emit('private_started', {'room_id': room_id, 'player_number': 1, 'opponent': p2['user'], 'opponent_color': p2['color'], 'your_color': p1['color']}, room=p1['sid'])
        socketio.emit('private_started', {'room_id': room_id, 'player_number': 2, 'opponent': p1['user'], 'opponent_color': p1['color'], 'your_color': p2['color']}, room=p2['sid'])
        socketio.emit('private_queue_update', {'count': len(private_queue)})
...
def handle_private_message(data):
    ...
    if not room:
        return
    room['messages'].append(data.copy())
...
def handle_request_game_state(data):
    ...
    if game_id in active_games:
        emit('game_state_update', active_games[game_id].get_state(), room=game_id)
        ...
        for game_id, game in list(active_games.items()):
            if game.running:
                ...
                if game.p1_score >= 5 or game.p2_score >= 5:
                    ...
                    socketio.emit('game_ended', {'winner': winner, 'p1_score': game.p1_score, 'p2_score': game.p2_score, 'winner_name': winner_name}, room=game_id)
                    socketio.emit('receive_message', {'msg': f'won Cosmic Pong and claimed the crown 👑', 'user': winner_name, 'color': game.player1_color if winner == 1 else game.player2_color, 'time': None, 'winner_name': winner_name}, broadcast=True)
                    game.running = False
                    del active_games[game_id]
